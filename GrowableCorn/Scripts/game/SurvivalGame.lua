...
local MapExportInterval = 20 -- 20 ticks | 0.5 seconds, for the external HTML world map
...
function SurvivalGame.server_onCreate( self )
	...
	self.sv.mapExportTimer = Timer()
	self.sv.mapExportTimer:start( 0 )

end

-- sm_overview: dumps live player world positions so an external HTML map can poll them
function SurvivalGame.sv_exportPlayerPositions( self )
	local playerData = {}
	for _, player in ipairs( sm.player.getAllPlayers() ) do
		if player.character and sm.exists( player.character ) then
			local pos = player.character.worldPosition
			local dir = player.character.direction
			playerData[#playerData+1] = {
				id = player.id,
				name = player.name,
				world = player.character:getWorld().id,
				x = pos.x,
				y = pos.y,
				z = pos.z,
				dirX = dir.x,
	...
	sm.json.save( { tick = sm.game.getServerTick(), players = playerData }, "$SURVIVAL_DATA/LocalBlueprints/live_playerpos.json" )
end

-- Best-effort Glowgorp ('glowbug') position export for the external map.
-- Wrapped in pcall so an API mismatch here can never break player tracking.
function SurvivalGame.sv_exportGlowbugs( self )
	local ok, err = pcall( function()
		local glowbugData = {}
		
		-- Get world from the first player
		...
		local world = players[1].character:getWorld()
		if not world then
			print("Could not get world from player")
			return
		end
		
		local worldId = world.id
		
		-- Get ALL units from the world (this searches the entire map)
		local allUnits = sm.unit.getAllUnits(world)
		...
		--print("Searching " .. #allUnits .. " total units for glowbugs...")
		
		local glowbugCount = 0
		for _, unit in ipairs( allUnits ) do
			if sm.exists( unit ) and unit.character and sm.exists( unit.character ) then
				local character = unit.character
				
				-- Get character type
				local charType = character:getCharacterType()
				
				-- Check if it's a glowbug
				local isGlowbug = false
				
				-- If unit_worm is defined (from unit_util.lua)
				if unit_worm ~= nil then
					isGlowbug = (charType == unit_worm)
				end
				
				-- If not, check string representation
				if not isGlowbug then
					local charTypeStr = tostring(charType or "")
					if charTypeStr == "unit_worm" or 
					   charTypeStr:find("worm") or 
					   charTypeStr:find("Glob") then
						isGlowbug = true
					end
				end
		...
		-- Save the data
		sm.json.save( { 
			tick = sm.game.getServerTick(), 
			glowbugs = glowbugData,
			totalGlowbugs = glowbugCount,
			timestamp = os.time()
		}, "$SURVIVAL_DATA/LocalBlueprints/live_glowbugs.json" )
		
		--print("Glowbug export completed. Found " .. glowbugCount .. " glowbugs out of " .. #allUnits .. " total units")
	end )
	
	if not ok then
		local msgerror = "Glowbug export failed (non-fatal): " .. tostring( err )
		print( msgerror )
...
function SurvivalGame.bindChatCommands( self )
	...
	if addCheats then
		...
		sm.game.bindChatCommand( "/spawn", { { "string", "unitName", true, autocomplete }, { "int", "amount", true } }, "cl_onChatCommand", "Spawn a unit: 'woc', 'tapebot', 'totebot', 'haybot', 'glowbug'" )
		sm.game.bindChatCommand( "/spawncornseedbot", {}, "cl_onChatCommand", "Spawn a corn seedbot" )
		...
		sm.game.bindChatCommand("/map", {}, "onMapCommand", "Opens the interactive map")
...
function SurvivalGame.server_onFixedUpdate( self, timeStep )
	...
	self.sv.mapExportTimer:tick()
	if self.sv.mapExportTimer:done() then
		self.sv.mapExportTimer:start( MapExportInterval )
		self:sv_exportPlayerPositions()
		self:sv_exportGlowbugs()
	end
...
function SurvivalGame.cl_onChatCommand( self, params )
	...
	elseif params[1] == "/seedsplease" then
		...
		self.network:sendToServer( "sv_giveItem", { player = sm.localPlayer.getPlayer(), item = ITEMS.obj_seed_corn, quantity = 20 } )
	...
	elseif params[1] == "/spawncornseedbot" then
		local rayCastValid, rayCastResult = sm.localPlayer.getRaycast( 100 )
		if rayCastValid then
			self.network:sendToServer( "sv_spawnUnit", {
				uuid = unit_seedbot,
				world = sm.localPlayer.getPlayer().character:getWorld(),
				position = rayCastResult.pointWorld,
				yaw = 0.0,
				amount = 1,
				customParams = { tetherPoint = rayCastResult.pointWorld, seedType = "corn" }
			} )
		end
